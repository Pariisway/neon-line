...
# The exact line we want to replace
old_line = '      const response = await fetch(`${TOKEN_SERVER_URL}/generate-token?channel=${channelName}&uid=${Math.floor(Math.random() * 100000)}`);'
...
if old_line in content:
    ...
    for i, line in enumerate(lines):
        if 'generate-token?channel=' in line:
